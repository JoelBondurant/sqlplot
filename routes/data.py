...
async def data(request):
	...
	if 'query_xid' in rquery:
		query_xid = rquery['query_xid']
	async with (request.app['pg_pool']).acquire(timeout=2) as pgconn:
		...
		''', query_xid, user_xid)) > 0
	query_url_key = f'{query_xid}.query_url'
	...
	if query_url is None:
		...
		async with aws.client('s3') as s3:
			query_url = await s3.generate_presigned_url('get_object',
				Params={'Bucket':'sqlplot', 'Key':f'query/{query_xid}.csv'},
				ExpiresIn=3600*24)
		await redis.set(query_url_key, query_url, expire=3600*24)
	else:
		query_url = query_url.decode()
	return aiohttp.web.json_response({'query_url': query_url})
